...
- **Migrations** run as a one-shot `migrate` compose service that reuses the `builder` image (it carries the Prisma CLI + schema + migration history) and runs `prisma migrate deploy`; `api`/`web` wait on its successful completion. The initial migration lives in `libs/database/prisma/migrations/`.
